...
public class NoInfantOrChildWithoutAdultRule implements TicketRule {
	...
	public ValidationStatus isValid(TicketTypeRequest[] ticketTypeRequests) {
		List<TicketTypeRequest.Type> typeList = Arrays.stream(ticketTypeRequests)
				.filter(ticketTypeRequest -> ticketTypeRequest.getNoOfTickets() > 0)
				.map(ticketTypeRequest -> ticketTypeRequest.getTicketType()).collect(Collectors.toList());
		...
		if ((isChildBooked || isInfantBooked) && !isAdultBooked) {
			return new ValidationStatus(ValidationStatus.Status.INVALID,
					"Infant or child ticket cannot be booked without adult");
		}
